...
module "ai" {
  ...
  for_each = { for k, v in var.vmss : k => "${v.name}-ai" if can(v.autoscale_profiles) }
...
module "vmss" {
  ...
  scale_in_policy              = try(each.value.scale_in_policy, null) # TODO
  scale_in_force_deletion      = try(each.value.scale_in_force_deletion, null) # TODO
  ...
  application_insights_id = can(each.value.autoscale_profiles) ? module.ai[each.key].application_insights_id : null
  # application_insights_id = can(each.value.autoscale_metrics) ? module.ai[each.key].application_insights_id : null

  autoscale_count_default       = try(each.value.autoscale_count_default, null)
  autoscale_count_minimum       = try(each.value.autoscale_count_minimum, null)
  autoscale_count_maximum       = try(each.value.autoscale_count_maximum, null)
  autoscale_notification_emails = try(each.value.autoscale_notification_emails, null) # TODO

  autoscale_profiles = try(each.value.autoscale_profiles, [])
