...
| interface | A array of map describing the intefaces configuration. Keys of the map are the names and values are { subnet\_id, private\_ip\_address, public\_ip, enable\_ip\_forwarding }. Example:<pre>[ // Only one interface in Panorama VM is supported <br>  {<br>    name                 = "mgmt"<br>    subnet_id            = ""<br>    private_ip_address   = ""         // Optional: If not set, use dynamic allocation<br>    public_ip            = "true"     // (optional|bool, default: "false")<br>    public_ip_name       = ""         // (optional|string, default: "")<br>    enable_ip_forwarding = "false"    // (optional|bool, default: "false")<br>    primary_interface    = "true"<br>  }<br>]</pre> | `any` | n/a | yes |
...
| os\_disk\_name | The suffix for disk name. | `string` | `null` | no |
...
| Name | Description |
|------|-------------|
| interface | Panorama network interface. The `azurerm_network_interface` object. |
| mgmt\_ip\_address | Panorama management IP address. If `public_ip` was `true`, it is a public IP address, otherwise a private IP address. |
