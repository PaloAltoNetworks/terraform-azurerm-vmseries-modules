...
| <a name="input_ssh_key"></a> [ssh\_key](#input\_ssh\_key) | Initial administrative SSH public key that allows for a key-pair authentication. If the `password` variable is also set, VM-Series will accept both authentication methods. | `string` | `null` | no |
