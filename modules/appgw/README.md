...
| `conditions` | a map of pre-conditions for a rule, for details see [property: rewrite\_sets.conditions](#property-rewritesetsconditions) | `map` | `null` | no |
...
#### property: rewrite\_sets.conditions
...
- [Palo Alto Networks VNet Module for Azure](#palo-alto-networks-vnet-module-for-azure)
  - [Rules property explained](#rules-property-explained)
    ...
    - [property: rewrite\\_sets](#property-rewrite\_sets)
      - [property: rewrite\\_sets.conditions](#property-rewrite\_setsconditions)
    - [property: url\\_path\\_maps](#property-url\_path\_maps)
  - [Usage](#usage)
    - [General](#general)
    - [`rules` property examples](#rules-property-examples)
...
[`rules`](#rules) | `any` | A map of rules for the Application Gateway.

## Module's Optional Inputs

Name | Type | Description
--- | --- | ---
[`zones`](#zones) | `list(string)` | A list of zones the Application Gateway should be available in.
[`domain_name_label`](#domain_name_label) | `string` | Label for the Domain Name.
[`managed_identities`](#managed_identities) | `list(string)` | A list of existing User-Assigned Managed Identities, which Application Gateway uses to retrieve certificates from Key Vault.
...
[`vmseries_ips`](#vmseries_ips) | `list(string)` | IP addresses of VMSeries' interfaces that will serve as backends for the Application Gateway.
...
[`ssl_policy_cipher_suites`](#ssl_policy_cipher_suites) | `list(string)` | A list of accepted cipher suites.
[`ssl_profiles`](#ssl_profiles) | `map(any)` | A map of SSL profiles that can be later on referenced in HTTPS listeners by providing a name of the profile in the `ssl_profile_name` property.
...
Name |  Description
--- | ---
[`public_ip`](#public_ip) | A public IP assigned to the Application Gateway
[`public_domain_name`](#public_domain_name) | Public domain name assigned to the Application Gateway
[`backend_pool_id`](#backend_pool_id) | The identifier of the Application Gateway backend address pool
...
A public IP assigned to the Application Gateway.

<sup>[back to list](#modules-outputs)</sup>
#### `public_domain_name`

Public domain name assigned to the Application Gateway.

<sup>[back to list](#modules-outputs)</sup>
#### `backend_pool_id`

The identifier of the Application Gateway backend address pool.
