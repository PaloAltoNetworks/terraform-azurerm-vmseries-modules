...
variable "bootstrap_storage" {
  ...
  - `storage_allow_vnet_subnet_ids` : (defaults to `[]`) whitelist containing the allowed vnet and associated subnets that are allowed to access the Storage Account. Note that the respective subnets require `enable_storage_service_endpoint` set to `true` to work properly.
  - `storage_allow_inbound_public_ips` : (defaults to `[]`) whitelist containing the allowed public IP subnets that can access the Storage Account. Note that you must always allow the public subnet of the source containing the bootstrap files to be able to upload them to the Storage Account.
